...
local function C_10()
local script = G2L["10"];
	local e = script.Parent.Parent.Parent:WaitForChild("Editor")
	...
	local function s(o)
		local i = string.lower(o)
	
		for _, a in ipairs(n) do
			if string.find(i, a) then
				return true, "Detected suspicious pattern: " .. a
			end
		end
	
		if string.find(i, "%d+%.%d+%.%d+%.%d+") then
			return true, "Detected raw IP address in code"
		end
	
		if string.find(i, 'https?://[^"]+discord[^"]*webhook[^"]*') then
			return true, "Detected potential Discord webhook URL"
		end
	...
	r.MouseButton1Click:Connect(
		function()
			...
			local d, u = s(h)
			if d then
				warn("SECURITY WARNING: Script contains potentially malicious code - " .. u)
				return
			end
	
			local f, c = pcall(loadstring, h)
	
			if f and type(c) == "function" then
				local p, y = pcall(c)
				if not p then
					warn("Runtime error: ", y)
				end
			else
				warn("Compile error: ", c)
			end
		end
	)
end;
